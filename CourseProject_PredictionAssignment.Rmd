...
First let us remove the colums `1:5` which seem to represent ids or (seeminlgy unimportant) timestamps for the data 
```{r, echo = T}
training.set.cleaned = training.set[, -c(1:5)]
```
...
We therefore end up with a trainig set of `54` observables
```{r, echo = T}
dim(training.set.cleaned)
```
for which the last one `classe` and all other columns are of type numeric or integer - see `str(training.set.cleaned)`.

Finally we apply the same procedure to the `testing.set` (but omit the prints).

#### Understand the data (level 2)

The training set has still `53` observables which might be significant for the prediction.
...
corMatrix = cor(training.set.cleaned[, -54])
corrplot(corMatrix, 
...
Finally to visualize this we at least provide a feature plot of the first `5` observables vs. the `classe` - it  also shows a non-trivial relation between each feature and `classe`.
...
We therefore take the following methods which we learned in class into consideration
